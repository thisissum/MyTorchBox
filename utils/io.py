...
def save_hdf5(path, keys, values):
    if isinstance(keys, list):
        assert len(keys) == len(values), "key and value must have save length"
        with h5py.File(path, 'w') as f:
            for i in range(len(keys)):
                f.create_dataset(keys[i], data=values[i])
    elif isinstance(keys, str):
        assert isinstance(values, list) == False, "only one key provided"
        with h5py.File(path, 'w') as f:
            f.create_dataset(keys, data=values)
    else:
        raise Exception("keys must be list or string")


def load_hdf5(path, keys):
